<template>
  <div class="app-container" style="text-align: center;">
    <upload-excel-component
      ...
      message="Arrastre histórico <<Info general>> aquí"
    />
...
import { createHistoricoGeneral } from '@/api/procesosDIEG/historico'
        ...
        for (const key in element) {
          ...
          if (key.substr(0, 2) === 'F_') { // Se validan los campos de fecha
            if (!isNaN(element[key])) { // Si es un numero
              ...
              element[key] = moment(this.numeroAFecha(element[key], true)).format('YYYY-MM-DD')
            }
          }
        }
        await createHistoricoGeneral(element).then((response) => {
          procesosCargados += 1
